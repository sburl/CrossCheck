...
wait_for_log_match() {
    local pattern="$1" since="$2" max_attempts="${3:-50}"
    local _attempt
...
test_codex_alone() {
    ...
    cleanup
    local TEST_DIR
    TEST_DIR=$(mktemp -d /tmp/hook-test-codex-alone-$$."XXXXXX")
    cd "$TEST_DIR"
    git init -q
...
test_git_then_codex() {
    ...
    cleanup
    local TEST_DIR
    TEST_DIR=$(mktemp -d /tmp/hook-test-git-then-codex-$$."XXXXXX")
    cd "$TEST_DIR"
    git init -q
    git config user.email "test@test.com"
    git config user.name "Test"
...
test_codex_then_git() {
    ...
    cleanup
    local TEST_DIR
    TEST_DIR=$(mktemp -d /tmp/hook-test-codex-then-git-$$."XXXXXX")
    cd "$TEST_DIR"
    git init -q
    git config user.email "test@test.com"
    git config user.name "Test"
