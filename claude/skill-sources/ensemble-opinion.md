...
description: 'Get multi-model opinions on a problem (Claude + Gemini + Critic)'
---
...
3. **Claude Agent** (subagent_type: general-purpose)
   - Prompt: "Run this command and return the output: `claude exec '<UNBIASED_PROMPT>' 2>&1 | tail -50`. Return the model's response, stripping metadata headers."
...
- **Critic**: [key points from claude_critic]
- **Gemini**: [key points, or "unavailable" if auth failed]
- **Claude**: [key points from claude response]
...
- If Gemini fails (auth): note it, continue with Claude + Critic
- If Claude fails (timeout): note it, continue with available responses
