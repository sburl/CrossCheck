# Claude Agent Review Prompts

**Created:** 2026-02-02-14-50
**Last Updated:** 2026-02-16-00-00

This file contains the exact prompts to use when coordinating with Claude agent for architecture review, test review, PR reviews, and repo assessments.

## Core Principle

**Reviewer model reviews, Claude writes.** The reviewer is a quality gate: it critiques Claude's work but does not implement code, suggest full rewrites, or drive design decisions.
...
### Spawning Reviewer Teammate (Claude-initiated)

**When:** After submitting a PR, before merge

**Using Agent Teams (Recommended):**

The primary Claude session spawns a reviewer teammate:
...
### Claude Pre-Review Information (Gate01.5)

**When:** BEFORE sending to Claude, after running /techdebt and /pre-pr-check (if available)

**What Claude should document:**
...
- Claude reviewer sends findings via **SendMessage** to the lead
- Lead assesses feedback, makes changes, and responds via **SendMessage**
- Loop continues automatically until Claude approves
- No manual copy-paste needed - agents communicate directly
- When Claude approves, lead merges and cleans up team

**Manual Alternative (if Agent Teams unavailable):**

If not using Agent Teams, use the manual process:
1. Run Claude review in separate terminal
2. Copy Claude response
...
```text
Hi Claude, I have been using an agent that doesn't think like you for the past few PRs and this repo has gotten a bit messy. I need your help to save me from uncaught errors and spaghetti code entropy! Please comprehensively assess this repo and develop a plan to get it back on track. Look for redundant/conflicting code and documentation. Suggest some concrete steps to make this code world class.
```

---

### Claude Refactor Response (Gate12)

**When:** After receiving Claude's comprehensive assessment

**Prompt to Claude:**

```text
A review agent comprehensively assessed the entire repo and had the below feedback. Please critically assess this feedback, change things as you see fit, and submit a pull request for the changes. If you disagree with any of the feedback please say so. You may merge only when the other agent explicitly tells you the PR can be merged and to where it can be merged. Feedback: {complete Claude response to prior message}
```

**Replace `{complete Claude response to prior message}` with Claude's full assessment.**
...
5. Claude reviews and sends feedback via SendMessage
6. Lead assesses feedback, makes changes
7. Lead responds to Claude via SendMessage
8. Claude re-reviews
9. Loop continues automatically until Claude approves
...
2. Use **Codex11** prompt in your Claude terminal
3. Copy Claude response
4. Use **Gate12** prompt with Claude's response
...
- If Claude says "looks good" but doesn't say "merge to X", ask for explicit approval

---

## Direct Claude Debugging (File-Based Pattern)
...
```bash
cat $TMPDIR/question.txt | claude exec --full-auto
```
...
Evaluate suggestions critically. Claude may identify real bugs but can occasionally misinterpret specifications. Always verify against authoritative sources.

### Best Practices

- **Provide complete code** - Don't truncate functions. Claude needs full context.
...
```bash
echo "Explain the difference between X and Y in this codebase" | claude exec --full-auto
```
...
Claude post-commit hooks provide automated review feedback after feature commits.
...
The hook logs a review prompt to `~/.claude/claude-commit-reviews.log` for asynchronous Claude review.

### Installation

```bash
# Install for current repo only:
~/.crosscheck/scripts/install-claude-hooks.sh

# Install globally for all repos:
~/.crosscheck/scripts/install-claude-hooks.sh --global
```
...
To skip Claude review for a single commit:
...
5. Review prompt is appended to `~/.claude/claude-commit-reviews.log`
6. Claude reviews are processed asynchronously via Claude

### Files

| File | Purpose |
|------|---------|
| `scripts/claude-commit-review.sh` | The review hook script |
| `scripts/install-claude-hooks.sh` | One-command installer |
