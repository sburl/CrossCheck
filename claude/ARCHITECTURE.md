...
*(Simplified view - see CLAUDE-PROMPTS.md for intermediate review stages)*
...
│  ┌────────────┐  ┌────────────┐  ┌─────────────┐        │
│  │ CI Tests   │  │ Bot Review │  │ Claude Review│        │
...
┌──────────────────────────────────────────────────────────┐
│  Phase 6: IMPROVE                                        │
│  Claude comprehensive repo assessment                     │
...
Claude (Reviewer) → Validates quality, catches bugs, gates merges
...
| **REVIEW** | Claude + CI + Bots | ~14% of issues |
...
Commit-msg → Enforce conventional commits format
    ↓
Post-commit → Track progress, log for Claude review
    ↓
...
7. /submit-pr → Runs /techdebt, /pre-pr-check
         ↓
8. If pass → Creates PR, invokes Claude
         ↓
9. Claude → Reviews PR, provides feedback
         ↓
10. Claude → Fixes issues, pushes updates
         ↓
11. Loop 9-10 until Claude approves
         ↓
12. Claude → "LGTM - merge to main"
         ↓
...
Claude → /repo-assessment
         ↓
Claude → Analyzes last 3 PRs:
    ├─ Common issues
    ├─ Workflow friction
    ├─ Missing automation
    └─ Improvement opportunities
         ↓
Claude → Generates improvement PR
         ↓
...
```
Claude writes → Commit reviews (Claude)
              ↓
         PR review layer 1 (Claude)
              ↓
...
```
Claude finds issue → Suggests fix
                  ↓
             Claude attempts fix
                  ↓
...
- **[CLAUDE-PROMPTS.md](CLAUDE-PROMPTS.md)** - Claude integration
