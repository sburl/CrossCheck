...
**Codex reviews, doesn't write.** Codex is a quality gate - it checks Claude's work but doesn't write code, suggest implementations, or drive the design. Claude Code does all work to its own satisfaction first.
...
### Spawning Codex Reviewer (Claude-initiated)

**When:** After submitting a PR, before merge

**Using Agent Teams (Recommended):**

Claude Code (as team lead) spawns a Codex reviewer teammate:

```text
Create an agent team for PR #{number} review. Spawn one Codex reviewer teammate with the prompt:
...
The Codex teammate runs autonomously, sends findings via SendMessage, and the lead responds with fixes. Loop continues until Codex approves.

---

### Claude Pre-Review Information (Claude01.5)
...
**Save this info to include when spawning the Codex reviewer teammate.**
...
3. Paste to Claude Code with: "A review agent shared this feedback: {response}"
4. Claude makes changes
5. Copy Claude's response
6. Paste to Codex with: "The agent made these updates: {response}"
...
### Claude Refactor Response (Claude12)
...
3. Submit PR
4. Spawn Codex reviewer teammate (see "Spawning Codex Reviewer" above)
...
2. Use **Codex11** prompt in Codex terminal
3. Copy Codex response
4. Use **Claude12** prompt with Codex's response
5. Claude creates refactor PR
...
The hook logs a review prompt to `~/.claude/codex-commit-reviews.log` for asynchronous Codex review.
...
5. Review prompt is appended to `~/.claude/codex-commit-reviews.log`
6. Codex reviews are processed asynchronously via Claude Code
