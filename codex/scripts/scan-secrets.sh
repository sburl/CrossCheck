...
if command -v rg >/dev/null 2>&1; then
    ...
    matches=$(rg -n --no-heading -g '!.git' -g '!node_modules' -g '!*.lock' -g '!*.min.js' \
        -g '!skills/security-review/SKILL.md' -g '!skill-sources/security-review.md' -g '!scripts/scan-secrets.sh' -g '!scripts/test-hook-behavior.sh' \
...
if [ "$SCAN_HISTORY" = true ]; then
    ...
    for pattern in "sk-proj-" "sk-ant-" "AKIA" "ghp_" "sk_live_" "AIza"; do
        ...
        found=$(git log --all -p -S "$pattern" --diff-filter=D --oneline -- ':!skills/security-review/SKILL.md' ':!skill-sources/security-review.md' ':!scripts/scan-secrets.sh' 2>/dev/null | head -5 || true)
        if [ -n "$found" ]; then
