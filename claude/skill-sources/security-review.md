...
7. **Secrets in AI agent logs (CRITICAL -- often overlooked)**
   ...
   Users paste secrets into Claude/Claude conversations. Agents read `.env` files into context. These get persisted in plaintext log files on disk.
...
This script uses exact regex patterns for all major providers (OpenAI, Anthropic, Google, AWS, GitHub, Stripe, Slack, etc.) and scans Claude/Claude log files at `~/.claude/projects/`. No false negatives for known key formats.
