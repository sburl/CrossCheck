...
if command -v git-filter-repo &>/dev/null; then
    git filter-repo --force \
        --name-callback "return name if email == b'$CORRECT_EMAIL' else b'$CORRECT_NAME'" \
        --email-callback "return email if email == b'$CORRECT_EMAIL' else b'$CORRECT_EMAIL'"
else
    # Fallback to filter-branch (only rewrite non-matching commits)
