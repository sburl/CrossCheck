...
Use Claude for security reviews, architecture analysis, and deep code inspection:

```bash
# In Claude:
/claude-delegate "Review PR #42 for security vulnerabilities"
/claude-delegate "Analyze database schema for optimization opportunities"
/claude-delegate "Identify code duplication across the codebase"
```

**How it works:**
1. Claude reads CLAUDE.md and injects context
2. Claude runs autonomously with `--full-auto` flag
3. Results reported back to Claude
4. Claude continues with Claude findings
...
mkdir -p ~/.claude/git-hooks/custom

# Create your hook
vim ~/.claude/git-hooks/custom/my-check.sh
```
...
```bash
#!/bin/bash
# ~/.claude/git-hooks/custom/my-check.sh
...
chmod +x ~/.claude/git-hooks/custom/my-check.sh

# Add to pre-commit hook
echo "" >> ~/.claude/git-hooks/pre-commit
echo "# Custom check" >> ~/.claude/git-hooks/pre-commit
echo "~/.claude/git-hooks/custom/my-check.sh || exit 1" >> ~/.claude/git-hooks/pre-commit
```
...
**Location:** `~/.claude/git-hooks/`

#### Pre-Commit Customization

```bash
vim ~/.claude/git-hooks/pre-commit
```
...
```bash
vim ~/.claude/git-hooks/commit-msg
```
...
git commit -m "feat: test claude review logging"

# Check the log
tail -5 ~/.claude/claude-commit-reviews.log
...
Commit: abc123d...
Message: feat: test claude review logging
[review prompt here]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
# Make writable
chmod 644 ~/.claude/claude-commit-reviews.log
...
ls -la ~/.claude/git-hooks/custom/my-hook.sh
# Should see: -rwxr-xr-x

# Make executable
chmod +x ~/.claude/git-hooks/custom/my-hook.sh

# Check it's called from main hook
grep "my-hook.sh" ~/.claude/git-hooks/pre-commit
# Should see the call

# Test manually
~/.claude/git-hooks/custom/my-hook.sh
...
- **[CLAUDE.md](CLAUDE.md)** - Daily workflow
