...
for repo in $(echo "$REPOS_JSON" | jq -r '.[].nameWithOwner'); do
    ...
    if [ "$mode" = "augment" ]; then
        ...
        # Write each block to a temp file (BSD awk can't take multi-line -v values).
        npm_file="$(mktemp)"; pip_file="$(mktemp)"; actions_file="$(mktemp)"
        [ $has_npm = 1 ]     && render_npm_block     "$repo_age_days" "$repo_major_days" > "$npm_file"
        [ $has_pip = 1 ]     && render_pip_block     "$repo_age_days" "$repo_major_days" > "$pip_file"
        [ $has_actions = 1 ] && render_actions_block "$repo_age_days" "$repo_major_days" > "$actions_file"

        mkdir -p .github
        awk -v npm_f="$npm_file" -v pip_f="$pip_file" -v acts_f="$actions_file" '
            function dump(file,    line) {
                while ((getline line < file) > 0) print line
                close(file)
            }
            /{{NPM_BLOCK}}/     { dump(npm_f);  next }
            /{{PIP_BLOCK}}/     { dump(pip_f);  next }
            /{{ACTIONS_BLOCK}}/ { dump(acts_f); next }
            { print }
        ' "$TEMPLATE" > .github/dependabot.yml
        rm -f "$npm_file" "$pip_file" "$actions_file"
    fi

    # Commit + PR. Bypass the global hookspath — CrossCheck's hooks expect to
    # run on CrossCheck-style repos with timestamp metadata, scan-leaks, etc.
    # These auxiliary pushes are mechanical edits to OTHER repos.
...
Policy: https://github.com/sburl/CrossCheck/blob/main/docs/rules/trust-model.md" >/dev/null
    ...
    # If there's an existing PR open on this branch, just nudge the user toward it instead of force-pushing.
    if existing_pr=$(gh pr list --repo "$repo" --head "$BRANCH" --state open --json url --jq '.[0].url' 2>/dev/null) && [ -n "$existing_pr" ]; then
        echo "  ℹ️  PR already exists for this branch: $existing_pr — skipping"
        skipped_has_cooldown=$((skipped_has_cooldown + 1))
        cd "$WORK"
        continue
    fi

    # Plain force-push: we own this branch space (auto-generated, never modified by humans).
    # Shallow clones can't use --force-with-lease since we don't have the remote refs.
    push_err=$($GIT push -u --force origin "$BRANCH" 2>&1) && push_ok=1 || push_ok=0
    if [ "$push_ok" = "1" ]; then
        if [ "$mode" = "augment" ]; then
        ...
        if pr_url=$(gh pr create --repo "$repo" --base "$default_branch" --head "$BRANCH" --title "$pr_title" --body "$pr_body" 2>&1); then
