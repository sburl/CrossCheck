...
if [ "$INSTALL_MODE" = "global" ]; then
    ...
    if [ ! -f ~/.codex/git-hooks/post-commit ]; then
        ...
        echo "  ℹ️  Dispatcher already exists: ~/.codex/git-hooks/post-commit"
    fi

    # Copy Codex review hook into post-commit.d/
    cp "$CROSSCHECK_DIR/scripts/codex-commit-review.sh" ~/.codex/git-hooks/post-commit.d/codex-review
    chmod +x ~/.codex/git-hooks/post-commit.d/codex-review

    # Set git to use global hooks (if not already set)
    if [ "$(git config --global core.hooksPath)" != "$HOME/.codex/git-hooks" ]; then
        git config --global core.hooksPath ~/.codex/git-hooks
    fi

    echo "✅ Installed globally! All repos will now use Codex hooks."
    echo "   Dispatcher: ~/.codex/git-hooks/post-commit"
    echo "   Codex hook: ~/.codex/git-hooks/post-commit.d/codex-review"
    echo "   To disable: rm ~/.codex/git-hooks/post-commit.d/codex-review"
...
echo ""
echo "📝 View Codex reviews: tail -f ~/.codex/codex-commit-reviews.log"
